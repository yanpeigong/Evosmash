...
class VideoDataset(Dataset):
    def __init__(self, frames, seq_len, sliding_step=1, bg_frame=None):
        ...
        self.bg_frame = bg_frame  # Optional background frame for background-aware TrackNet.
...
class CoordinateDataset(Dataset):
    """Coordinate dataset used by InpaintNet."""

    def __init__(self, coords, masks, seq_len):
        self.coords = coords
        self.masks = masks
        self.seq_len = seq_len
        # Pad the tail so the output sequence length still matches the video length.
        pad_len = seq_len - 1
...
class BallTracker:
    def __init__(self):
        ...
        # get_model already includes the corrected TrackNet construction.
        ...
        if INPAINTNET_PATH:
            try:
                ...
                print(f"Warning: Failed to load InpaintNet: {e}")

    def infer(self, video_path):
        frames, fps = generate_frames(video_path)
        video_len = len(frames)

        # Build the median background frame if background mode is enabled.
        bg_frame = None
        if self.bg_mode:
            print("Calculating background median...")
            # Sample at most 50 frames for speed.
            if len(frames) > 50:
        ...
        with torch.no_grad():
            for x, indices in tqdm(loader, desc="TrackNet"):
                x = x.to(self.device)
                y_pred = self.tracknet(x).detach().cpu()  # (N, L, H, W)
        ...
        # Parse coordinates from the averaged heatmaps.
        ...
        if self.inpaintnet:
            ...
            with torch.no_grad():
                for c_seq, m_seq, indices in tqdm(coord_loader, desc="InpaintNet"):
                    ...
                    for batch_index in range(c_pred.shape[0]):
                        idx = indices[batch_index]  # Shape: (seq_len,)

                        # Drop padded indices that extend past the true video length.
                        valid_mask = idx < video_len
                        valid_idx = idx[valid_mask]

                        # Only accumulate when there are valid indices left.
                        if len(valid_idx) > 0:
                            # Apply the same mask to the predicted coordinates and weights.
            ...
            refined_coords = coord_accum / (coord_count + 1e-6)
            refined_coords = refined_coords.numpy()
            final_traj = []
            for frame_index in range(video_len):
            ...
            return final_traj, fps

        return list(zip(pred_dict['X'], pred_dict['Y'])), fps
